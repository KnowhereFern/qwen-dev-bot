...
describe('guided installer', () => {
  ...
  it('launches the packaged CLI outside the harness checkout', async () => {
    ...
    expect(receipt.stdout.trim()).toBe('1.0.0-rc.20');
  });
...
`);
    ...
    const immutableRuntime = path.join(state, 'controller', 'installed', '1.0.0-rc.20', 'node_modules', 'qwen-dev-bot');
  ...
  it('leaves a recovery receipt when an optional external setup step fails', async () => {
    ...
    expect(existsSync(path.join(state, 'controller', 'installed', '1.0.0-rc.20', 'node_modules', 'qwen-dev-bot', 'bin', 'qwen-harness-launcher.mjs'))).toBe(true);
